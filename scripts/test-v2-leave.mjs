...
async function ready(){for(let i=0;i<150;i++){try{if((await fetch(`${base}/employe/conges/`)).ok)return;}catch{}await new Promise(resolve=>setTimeout(resolve,200));}throw new Error("Serveur indisponible");}
try{await ready();let r=await req("/api/admin/auth/step-1",{method:"POST",body:{secret:s1}});r=await req("/api/admin/auth/step-2",{method:"POST",body:{secret:s2}});check(r.response.ok,"session admin");
r=await req("/api/admin/v2/employees",{method:"POST",body:{firstName:"Omar",lastName:`Conges-${suffix}`,password:"1234",jobTitle:"Technicien",language:"fr"}});employeeId=r.data.id;check(r.response.status===201,"employé créé");
r=await req("/api/admin/v2/holidays",{method:"POST",body:{date:"2026-09-07",label:"Jour férié test"}});check(r.response.status===201,"jour férié configuré");
r=await req("/api/v2/auth/login",{method:"POST",body:{firstName:"Omar",lastName:`Conges-${suffix}`,password:"1234"},jar:jars.employee,ip:"198.51.100.102"});check(r.response.ok,"connexion employé");
r=await req("/api/v2/employee/leave",{method:"POST",body:{startDate:"2026-09-04",endDate:"2026-09-08",comment:"Repos"},jar:jars.employee,ip:"198.51.100.102"});check(r.response.status===201&&r.data.workingDays===2,"calcul hors week-end et jour férié");const leaveId=r.data.id;
