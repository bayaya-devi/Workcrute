...
for (const id of ["home", "process", "about", "conditions"]) assert.ok(home.includes(`id="${id}"`), id);
...
const context = { window: {}, navigator: { languages: ["fr"] }, localStorage: { getItem: () => "fr" }, document: { readyState: "loading", addEventListener() {} } };
...
assert.ok(backend.includes('pathname === "/api/v2/applicants" && request.headers.get("origin") === "https://bayaya-devi.github.io"'));
