...
const [applicants,auth,headers,identity]=await Promise.all([read("src/v2-applicants.js"),read("src/v2-auth.js"),read("public/_headers"),read("public/admin/securite/index.html")]);
...
check(auth.includes("(!password && !existing) || password.length > 256")&&!identity.includes('minlength='),"mot de passe initial non vide, modification facultative sans longueur minimale imposée");
