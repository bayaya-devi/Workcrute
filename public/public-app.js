(() => {
  ...
  const sectionHref = id => route === "/" ? `#${id}` : `${href("/")}#${id}`;
  ...
  const header = () => `
    ...
    <header class="wc-header">
      <div class="wc-container wc-header-inner">
        ...
        <nav class="wc-nav" data-i18n-aria="main_navigation" aria-label="Navigation principale">
          <a href="${sectionHref("home")}" data-nav="home" data-i18n="nav_home">${t("nav_home")}</a>
          <a href="${sectionHref("process")}" data-i18n="nav_process">${t("nav_process")}</a>
          <a href="${sectionHref("about")}" data-i18n="nav_about">${t("nav_about")}</a>
          <a href="${sectionHref("conditions")}" data-i18n="footer_terms">${t("footer_terms")}</a>
        </nav>
    ...
    <div class="wc-mobile-drawer" data-mobile-drawer aria-hidden="true">
      ...
      <aside class="wc-drawer-panel" data-i18n-aria="mobile_navigation" aria-label="Navigation mobile">
        ...
        <nav class="wc-drawer-nav"><a href="${sectionHref("home")}" data-i18n="nav_home">${t("nav_home")}</a><a href="${sectionHref("process")}" data-i18n="nav_process">${t("nav_process")}</a><a href="${sectionHref("about")}" data-i18n="nav_about">${t("nav_about")}</a><a href="${sectionHref("conditions")}" data-i18n="footer_terms">${t("footer_terms")}</a></nav>
  ...
  function scoreFaq(entry, query, language) {
    ...
    const phraseBoost = keywords.some((keyword) =>
      normalize(query).includes(normalize(keyword)),
    )
      ? 2
      : 0;
    return (
      (overlap + phraseBoost) /
      Math.max(2, Math.sqrt(queryTokens.size * targetTokens.size))
    );
  }
  function findFaq(query, language = i18n.getLanguage()) {
    return faq
      .filter((entry) => entry.is_active)
      .map((entry) => ({ entry, score: scoreFaq(entry, query, language) }))
      .sort((a, b) => b.score - a.score);
  }
  function localized(entry, field, language = i18n.getLanguage()) {
    return entry[`${field}_${language}`] || entry[`${field}_fr`];
  }
  ...
  function setupChat() {
    ...
    const suggestions = (entries) => {
      ...
      wrap.className = "wc-chat-suggestions";
      entries.forEach((entry) => {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "wc-chat-suggestion";
        button.textContent = localized(entry, "question");
  ...
  function bindAccordions(scope = document) {
    scope.querySelectorAll(".wc-faq-question").forEach((button) =>
      button.addEventListener("click", () => {
        const item = button.closest(".wc-faq-item");
  ...
  function renderFaq() {
    const renderPreviews = () =>
      document.querySelectorAll("[data-faq-preview]").forEach((target) => {
        target.innerHTML = faq.slice(0, 6).map(faqMarkup).join("");
        bindAccordions(target);
      });
    renderPreviews();
    const target = document.querySelector("[data-faq-list]");
    if (!target) {
      document.addEventListener("workcrute:language", renderPreviews);
      return;
    }
    ...
    const render = (query) => {
      ...
      const items = query
        ? findFaq(query, language)
            .filter((item) => item.score > 0.08)
            .slice(0, 30)
            .map((item) => item.entry)
        : faq.slice(0, 30);
      target.innerHTML = items.length
        ? items.map(faqMarkup).join("")
        : `<div class="wc-empty"><div class="wc-empty-icon">?</div><h3 data-i18n="no_faq">${t("no_faq")}</h3></div>`;
      bindAccordions(target);
    };
    if (input && !input.dataset.faqBound) {
      input.dataset.faqBound = "true";
  ...
  async function renderJobs() {
    ...
    const load = async (params) => {
      ...
      target.innerHTML = [1, 2, 3]
        .map(
          () =>
            `<div class="wc-card wc-card--pad"><div class="wc-skeleton" style="width:65%;height:24px"></div><div class="wc-skeleton" style="margin-top:16px;height:60px"></div></div>`,
        )
        .join("");
      try {
        ...
        target.innerHTML = jobs.length
          ? jobs
              .slice(0, preview ? 6 : 50)
              .map((job) => jobCard(job, Boolean(list)))
              .join("")
          : empty;
        if (count) count.textContent = String(jobs.length);
        const stat = document.querySelector("[data-stat-jobs]");
        if (stat) stat.textContent = String(jobs.length);
      } catch {
        target.innerHTML = `<div class="wc-error-state"><div class="wc-empty-icon">!</div><h3>${t("load_error")}</h3><button class="wc-button wc-button--primary" type="button" data-retry-jobs>${t("retry")}</button></div>`;
        target
          .querySelector("[data-retry-jobs]")
          ?.addEventListener("click", () => load(params));
      } finally {
        target.removeAttribute("aria-busy");
      }
    };
    if (searchForm && !searchForm.dataset.jobsBound) {
      searchForm.dataset.jobsBound = "true";
    ...
    if (resetButton && !resetButton.dataset.jobsBound) {
      resetButton.dataset.jobsBound = "true";
    ...
    if (mobileButton && !mobileButton.dataset.jobsBound) {
      mobileButton.dataset.jobsBound = "true";
  ...
  async function renderStats() {
    ...
    try {
      ...
      [
        ["candidates", "[data-stat-candidates]"],
        ["companies", "[data-stat-companies]"],
        ["jobs", "[data-stat-jobs]"],
        ["applications", "[data-stat-applications]"],
      ].forEach(([key, selector]) => {
        const node = document.querySelector(selector);
        if (node)
          node.textContent = Number.isFinite(Number(stats[key]))
            ? Number(stats[key]).toLocaleString(i18n.getLanguage())
            : "—";
      });
    } catch {
      document
        .querySelectorAll("[data-public-stats] strong")
  ...
  async function renderJobDetail() {
    ...
    if (!id) {
      target.innerHTML = `<div class="wc-empty"><h1>${t("job_not_found")}</h1><p>${t("job_not_found_copy")}</p><a class="wc-button wc-button--primary" href="${href("/offres")}">${t("back_jobs")}</a></div>`;
      return;
    }
    try {
      ...
      const skills = (() => {
        try {
          return JSON.parse(job.required_skills || "[]");
        } catch {
          return [];
        }
      })();
      ...
      target
        ...
        ?.addEventListener("click", async (event) => {
          ...
          try {
            ...
            if (/auth|connect/i.test(String(error.message)))
      ...
      target
        .querySelector("[data-share-job]")
        ?.addEventListener("click", async () => {
          try {
            if (navigator.share)
              await navigator.share({ title: job.title, url: location.href });
  ...
  function toast(message, kind = "info") {
    let region = document.querySelector(".wc-toast-region");
    if (!region) {
      region = document.createElement("div");
      region.className = "wc-toast-region";
      region.setAttribute("aria-live", "polite");
      document.body.append(region);
    }
    const node = document.createElement("div");
    node.className = `wc-toast wc-toast--${kind}`;
  ...
  function setupForms() {
    document.querySelectorAll("[data-password-toggle]").forEach((button) => {
      ...
      button.addEventListener("click", () => {
        const input = button
          .closest(".wc-password-wrap, .password-field")
          ?.querySelector("input");
        if (!input) return;
    ...
    heroSearch?.addEventListener("submit", (event) => {
      event.preventDefault();
      const params = new URLSearchParams(new FormData(heroSearch));
      location.href = `${href("/offres")}?${params}`;
    });
    const wizard = document.querySelector(
      "[data-signup], [data-recruiter-signup]",
    );
    const syncStepLabel = () => {
      if (!wizard) return;
      ...
      if (label)
        label.textContent = `${t("step")} ${active + 1} ${t("of")} ${steps.length}`;
    };
    wizard
      ?.querySelectorAll("[data-next], [data-back]")
      .forEach((button) =>
        button.addEventListener("click", () => setTimeout(syncStepLabel, 0)),
      );
