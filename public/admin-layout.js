(() => {
  ...
  const links = [
    ["dashboard","/admin/tableau-de-bord/","dashboard","⌂"],["activity","/admin/journal-activite/","activity","↻"],
    ["candidates","/admin/demandeurs/","candidates","♙"],["recruiters","/admin/recruteurs/","recruiters","♜"],
    ["companies","/admin/entreprises/","companies","▦"],["jobs","/admin/offres/","jobs","▤"],
    ["applications","/admin/candidatures/","applications","✓"],["interviews","/admin/entretiens/","interviews","◷"],
    ["questionnaires","/admin/questionnaires/","questionnaires","?"],["notifications","/admin/notifications/","notifications","♢"],
    ["audit","/admin/journal-activite/","audit","≡"],["settings","/admin/parametres/","settings","⚙"],
    ["security","/admin/securite/","security","◈"],
  ];
  const nav = links.map(([id,href,key,icon], index) => `${index === 2 || index === 9 ? `<p class="adm-nav-label">${index === 2 ? "Plateforme" : "Administration"}</p>` : ""}<a href="${href}" ${page === id ? 'aria-current="page"' : ""}><span aria-hidden="true">${icon}</span><span data-adm-i18n="${key}">${key}</span></a>`).join("");
