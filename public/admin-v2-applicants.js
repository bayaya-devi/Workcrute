(() => {
  ...
  async function openDetail(id) {
    ...
    const fields = ['first_name','last_name','gender','email','phone','city','country','professional_title','domain','experience_level','availability','motivation','preferred_language'].map(key=>[t(key), ['gender','domain','experience_level','availability','preferred_language'].includes(key)?(key==='gender'?(item.gender==='female'?t('female'):item.gender==='male'?t('male'):'—'):(key==='domain'&&item.domain_other?item.domain_other:t(item[key]))):item[key]]);
