...
const leaveCopies={fr:{approved:"Votre demande de congé est approuvée",refused:"Votre demande de congé est refusée"},en:{approved:"Your leave request has been approved",refused:"Your leave request has been declined"},ar:{approved:"تمت الموافقة على طلب الإجازة",refused:"تم رفض طلب الإجازة"}};
...
export async function processV2EmployeeEmails(env,limit=25){const rows=(await env.DB.prepare("SELECT * FROM v2_employee_email_outbox WHERE ((status IN ('pending','failed') AND next_attempt_at<=CURRENT_TIMESTAMP) OR (status='processing' AND updated_at<datetime('now','-5 minutes'))) AND attempts<max_attempts ORDER BY created_at LIMIT ?").bind(limit).all()).results||[];for(const row of rows){const claimed=await env.DB.prepare("UPDATE v2_employee_email_outbox SET status='processing',updated_at=CURRENT_TIMESTAMP WHERE id=? AND ((status IN ('pending','failed') AND next_attempt_at<=CURRENT_TIMESTAMP) OR (status='processing' AND updated_at<datetime('now','-5 minutes'))) AND attempts<max_attempts").bind(row.id).run();if(!claimed.meta?.changes)continue;try{const request=await env.DB.prepare("SELECT start_date,end_date,admin_comment FROM v2_leave_requests WHERE id=?").bind(row.resource_id).first(),copy=leaveCopies[row.language]||leaveCopies.fr,event=row.event.replace('leave_','');await sendTransactionalEmail(env,{to:row.recipient,subject:`[Workcrute] ${copy[event]}`,text:`${copy[event]}.\n${request?.start_date||''} - ${request?.end_date||''}${request?.admin_comment?`\n${request.admin_comment}`:''}`});await env.DB.prepare("UPDATE v2_employee_email_outbox SET status='sent',attempts=attempts+1,last_error=NULL,sent_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(row.id).run();}catch(error){const attempts=Number(row.attempts)+1;await env.DB.prepare("UPDATE v2_employee_email_outbox SET status='failed',attempts=?,last_error=?,next_attempt_at=datetime('now',?),updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(attempts,String(error).slice(0,300),`+${Math.min(3600,60*2**attempts)} seconds`,row.id).run();}}}
