...
import { employeeInvoices } from "./v2-invoices.js";
...
export async function v2Employee(request,env,path){
  ...
  if(path==="/api/v2/employee/overview"&&request.method==="GET"){
    ...
    const [leave,invoices]=await Promise.all([employeeLeaveSummary(env,session.account_id),employeeInvoices(env,session.account_id)]);
    return json({profile,leave,invoices:{count:invoices.items.length,recent:invoices.items.slice(0,1),requests:invoices.requests.slice(0,1)}});
  }
