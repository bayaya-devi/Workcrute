import { hashV2Password } from "./v2-auth.js";
...
async function update(request,env,id){
  ...
  if(password){if(password.length>256)return bad("Mot de passe invalide.",422);const salt=token();statements.push(env.DB.prepare("UPDATE v2_accounts SET password_hash=?,password_salt=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(await hashV2Password(password,salt),salt,id),env.DB.prepare("UPDATE v2_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE account_id=?").bind(id));}
